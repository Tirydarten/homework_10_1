...
def test_mask_card_func(new_string_card: str) -> None:
...
@pytest.mark.parametrize(
    "number, number_card",
    [
        ("Visa Gold 5999414228426553", "** **** 6553"),
        ("Visa Classic 5999414228416354", "** **** 6354"),
        ("Visa Premium 5999414228476356", "** **** 6356"),
    ],
)
def test_get_mask_card_number_2(number: str, number_card: str) -> None:
...
@pytest.mark.parametrize(
    "input_card_num", ["Visa Gold 59994142", "Visa Classic 59994142284263128888", "Visa Premium 0"]
)
def test_mask_card_format(input_card_num: str, expected: str = "Неверный формат номера карты") -> None:
...
def test_mask_card_num_trace(card_num_trace: str) -> None:
...
def test_mask_card_duration(mask_card_duration: str) -> None:
...
@pytest.mark.parametrize(
    "number",
    [
        "Visa Gold 5999414228426553",
        "Visa Classic 5999414228416354",
        "Visa Premium 5999414228476356",
    ],
)
def test_mask_card_number(number: str, num_isdigit: bool = True, len_num: int = 21, expected: int = 16) -> None:
...
def test_get_mask_card_empty(empty_card_numb: str, expected: str = "Ошибка типа данных") -> None:
...
def test_get_mask_card_empty_2() -> Any:
...
@pytest.mark.parametrize("new_string_account", ("Счет 7365410843013581228", "Счет 73654108430188877678888"))
def test_get_mask_account_format(new_string_account: str, expected: str = "Ошибка в номере счета карты") -> None:
...
def test_get_mask_account_empty(empty_account_numb: str, expected_account: str = "Ошибка типа данных") -> None:
    """Проверка, что функция корректно обрабатывает входные строки,
    где отсутствует номер счета"""
    assert get_mask_account(empty_account_numb) == expected_account


def test_get_mask_account_empty_2(expected: str = "Ошибка типа данных") -> None:
    """Проверка, что функция корректно обрабатывает входные строки,
    где отсутствует номер счета"""
